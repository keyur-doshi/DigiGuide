...
<body>
  ...
  <main>
    ...
    <form id="assignmentForm" action="http://localhost/dbms/assteacher.php" method="POST">
      <div>
        <label for="assignmentName">Assignment Name:</label>
        <input type="text" id="assignmentName" name="namez" required>
      </div>
      <div>
        <label for="dueDate">Due Date:</label>
        <input type="date" id="dueDate" name="datez" required>
      </div>
      <div>
        ...
        ?>
        <select id="courseCode" name="codez" required>
    ...
    echo "<option value=\"$code\">$code</option>";
      ...
      <div>
        <label for="topicsDescription">Topics Description:</label>
        <textarea id="topicsDescription" rows="4" name="descz" required></textarea>
      </div>

      <div>
        <label for="gform">Submission Link:</label>
        <input type="text" id="gform" name="gform" required>
      </div>
      <div id="btn">
    ...
    <br/><center><h2>All Your Assignments</h2></center>
    <?php
        $ssql = "select * from assignments where prof='{$_SESSION['leg']}'";
        ...
        if (mysqli_num_rows($extraRes) > 0)
        {
          ...
          <thead>
              <tr>
                  ...
                  <th style="padding: 10px; border: 1px solid #ddd;">Submission Link</th>
              </tr>
          </thead>
          <tbody>';
      
      while ($row = mysqli_fetch_assoc($extraRes)) {
          echo "<tr>";
          echo "<td style='padding: 10px; border: 1px solid #ddd;'>{$row['course_code']}</td>";
          echo "<td style='padding: 10px; border: 1px solid #ddd;'>{$row['due_date']}</td>";
          echo "<td style='padding: 10px; border: 1px solid #ddd;'>{$row['assignment_name']}</td>";
          echo "<td style='padding: 10px; border: 1px solid #ddd; width: 150px;'>{$row['description']}</td>"; // Adjust the width as needed
          echo "<td style='padding: 10px; border: 1px solid #ddd;'>{$row['gform']}</td>";
          echo "</tr>";
      }
